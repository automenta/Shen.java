...
public class MicroBench {
    public static void main(String[] args) throws Throwable {
        ...
        Primitives.set("*debug*", true);
        Shen.eval("(defun fib (n) (if (<= n 1) n (+ (fib (- n 1)) (fib (- n 2)))))");
        ...
        Shen.eval("(defun my-cons (a b) (cons a b))");
        ...
        Shen.eval("(defun map (f x) (if (cons? x) (cons (f (hd x)) (map f (tl x))) ()))");
        bench("(map (+ 1) (cons 1 (cons 2 (cons 3 ()))))", times);

        Shen.eval("(defun inc (x) (+ 1 x))");
        ...
        Shen.eval("(defun my-cons (x) ((cons 1) x))");
    ...
    static void bench(String test, int times) throws Throwable {
        Object kl = KLReader.read(new StringReader(test)).get(0);
    ...
    static void bench(Callable<?> code, int times) throws Exception {
        ...
        for (int i = 0; i < times; i++) System.out.println(Numbers.maybeNumber(code.call()));
